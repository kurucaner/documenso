...
- **`NEXT_PRIVATE_INTERNAL_SECRET`**: Shared secret for authenticated internal API routes under `/api/internal/*` (signup invites and direct user provisioning). When set together with `NEXT_PUBLIC_DISABLE_SIGNUP="true"`, invited or provisioned users can still create email/password accounts via the internal APIs.
