...
- **`NEXT_PRIVATE_INTERNAL_SECRET`**: Shared secret for authenticated internal API routes under `/api/internal/*` (signup invites, direct user provisioning, and team API token creation). When set together with `NEXT_PUBLIC_DISABLE_SIGNUP="true"`, invited or provisioned users can still create email/password accounts via the internal APIs.
