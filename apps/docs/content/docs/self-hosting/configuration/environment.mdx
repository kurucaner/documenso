...
Customize the application name, logos, and other branding shown in the UI, browser tab titles, emails, and PWA manifest.

| Variable | Description | Default |
| -------- | ----------- | ------- |
| `NEXT_PUBLIC_APP_NAME` | Application name shown in page titles, emails, and UI copy | `Documenso` |
| `NEXT_PUBLIC_APP_COMPANY_NAME` | Legal company name used in meta tags and email footers | App name followed by `, Inc.` |
| `NEXT_PUBLIC_APP_DESCRIPTION` | Default meta description | `Sign and manage documents with {your app name}.` |
| `NEXT_PUBLIC_APP_LOGO_URL` | Custom logo URL or path (e.g. `/static/logo.png`) | Built-in logo at `/static/logo.png` |
| `NEXT_PUBLIC_APP_FAVICON_URL` | Custom favicon URL or path | Built-in favicons in `apps/remix/public/` |
| `NEXT_PUBLIC_APP_HIDE_POWERED_BY` | Hide "sent using {APP_NAME}" in email footers when set to `true` | `false` |
| `NEXT_PUBLIC_APP_HIDE_EMAIL_LOGO` | Hide the default app logo in emails when set to `true` (org/team custom branding logos still show) | `false` |
| `NEXT_PUBLIC_APP_COMPANY_ADDRESS` | Company address shown in email footers when custom branding is not enabled | `2261 Market Street, #5211, San Francisco, CA 94114, USA` |
| `NEXT_PUBLIC_APP_ATTRIBUTION_URL` | URL used for marketing links in signing flows and email attribution | `https://documen.so/mail-footer` |

If `NEXT_PRIVATE_SMTP_FROM_NAME` is not set, outgoing emails use `NEXT_PUBLIC_APP_NAME` as the sender display name.

Replace favicon PNG files in `apps/remix/public/` or point `NEXT_PUBLIC_APP_FAVICON_URL` at your own assets. The web app manifest is generated dynamically from these settings.
...
| `NEXT_PRIVATE_INTERNAL_SECRET`               | Shared secret for internal API routes under `/api/internal/*`                         |         |
| `NEXT_PRIVATE_SIGNUP_INVITE_EXPIRY_DAYS`     | Default number of days before a signup invite expires (max 30)                          | `7`     |
| `NEXT_PRIVATE_DEFAULT_ORGANISATION_NAME`     | Default display name for personal organisations created at signup                     |         |
| `NEXT_PRIVATE_DEFAULT_TEAM_NAME`           | Default display name for personal teams created at signup                             |         |
...
- **`NEXT_PRIVATE_INTERNAL_SECRET`**: Shared secret for authenticated internal API routes under `/api/internal/*` (signup invites, direct user provisioning, organisation team creation, and team API token creation). When set together with `NEXT_PUBLIC_DISABLE_SIGNUP="true"`, invited or provisioned users can still create email/password accounts via the internal APIs.
...
- **`NEXT_PRIVATE_DEFAULT_ORGANISATION_NAME`**: Default display name for personal organisations created during signup. Overridden by per-invite `organisationName` when set via the [Signup Invites](/docs/self-hosting/configuration/signup-invites) API.
- **`NEXT_PRIVATE_DEFAULT_TEAM_NAME`**: Default display name for personal teams created during signup. Overridden by per-invite `teamName` when set via the invite API.
...
# Branding (optional)
# NEXT_PUBLIC_APP_NAME="My Company Sign"
# NEXT_PUBLIC_APP_COMPANY_NAME="My Company, Inc."
# NEXT_PUBLIC_APP_COMPANY_ADDRESS="123 Example St, City, ST 00000"
# NEXT_PUBLIC_APP_LOGO_URL="/static/logo.png"
# NEXT_PUBLIC_APP_FAVICON_URL="/favicon-32x32.png"
# NEXT_PUBLIC_APP_HIDE_EMAIL_LOGO="true"
# NEXT_PUBLIC_APP_HIDE_POWERED_BY="true"
