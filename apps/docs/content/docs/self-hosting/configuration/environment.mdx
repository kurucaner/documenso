...
- **`NEXT_PRIVATE_INTERNAL_SECRET`**: Shared secret for authenticated internal API routes under `/api/internal/*`. Currently used by the [Signup Invites](/docs/self-hosting/configuration/signup-invites) API. When set together with `NEXT_PUBLIC_DISABLE_SIGNUP="true"`, invited users can still create email/password accounts.
