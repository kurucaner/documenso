...
Datadog Real User Monitoring (RUM) is **disabled unless `NEXT_PUBLIC_DD_RUM_APPLICATION_ID` and `NEXT_PUBLIC_DD_CLIENT_TOKEN` are set**. When enabled, the web app initializes `@datadog/browser-rum` on the client, syncs authenticated users and route views, and reports non-404 errors from the root error boundary. Signing and direct-template URL tokens are redacted before events are sent.

Without `NEXT_PUBLIC_DD_RUM_PROXY_URL`, the browser sends RUM events **directly** to Datadog intake for your site. That works for quick setup but may be blocked by ad blockers or strict CSP — set a proxy URL for production.

When using a proxy, point `NEXT_PUBLIC_DD_RUM_PROXY_URL` at a service that forwards obfuscated requests to Datadog browser intake (not the PostHog `/ingest` route). If you co-host with PropertyOS, you can use its `apps/proxy` service (e.g. `http://localhost:8082`) and add your Documenso origin to the proxy's `CORS_ORIGINS`. The browser calls `{proxyUrl}/ingest?t=…`; the proxy forwards to `browser-intake-*` for your Datadog site.
...
| `NEXT_PUBLIC_DD_RUM_PROXY_URL` | No | RUM intake proxy base URL (no trailing slash). Omit for direct Datadog intake. |
| `NEXT_PUBLIC_DD_SERVICE` | No | RUM service name (defaults to `documenso-web`) |
...
NEXT_PUBLIC_DD_RUM_APPLICATION_ID=<rum-application-id>
NEXT_PUBLIC_DD_CLIENT_TOKEN=<client-token>
# Optional — omit for direct Datadog intake; recommended for production:
# NEXT_PUBLIC_DD_RUM_PROXY_URL=https://rum-proxy.example.com
# NEXT_PUBLIC_DD_SERVICE=documenso-web
NEXT_PUBLIC_DD_ENV=production
NEXT_PUBLIC_DD_SITE=us5.datadoghq.com
...
2. Set application id and client token; optionally deploy a RUM intake proxy and allow your Documenso origin in its CORS configuration.
3. Restart Documenso.
4. Open the web app → in browser DevTools **Network**, confirm POST requests to Datadog intake (`browser-intake-*`) or, when using a proxy, `{NEXT_PUBLIC_DD_RUM_PROXY_URL}/ingest?t=…`.
5. In Datadog → **Digital Experience → RUM → Sessions**, confirm sessions with service `documenso-web` (or your `NEXT_PUBLIC_DD_SERVICE`) and `NEXT_PUBLIC_DD_ENV`.
6. Sign in → confirm the session user shows your account email/id.
7. Navigate between pages → confirm view updates.
8. Unset application id or client token and restart → confirm no RUM network traffic and no console errors.
